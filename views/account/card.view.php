...
<div class="card">
    ...
    <div class="card-body">
                ...
                <div class="front">
                    ...
                    <svg version="1.1" id="cardfront" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 750 471" style="enable-background:new 0 0 750 471;" xml:space="preserve">
                        <g id="Front">
                            ...
                            <text transform="matrix(1 0 0 1 60.106 295.0121)" id="card-number" class="card-number card-text">{{card.card_number}}</text>
                            <text transform="matrix(1 0 0 1 54.1064 428.1723)" id="svgname" class="card-name card-text">{{card.card_name|upper}}</text>
                            ...
                            <g>
                                {% if card.status == 'active' %}
                                <text transform="matrix(1 0 0 1 574.4219 433.8095)" id="svgexpire" class="card-expire card-text">{{card.card_validity}}</text>
                                {% else %}
        ...
        <!-- Card container ends -->
        {% if request.user.has_active_card %}
            ...
            {% csrf_token %}
             <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input type="number" class="form-control" name="amount" value=3000 required readonly disabled>
              </div>
        ...
        <div class="mt-5 mb-5 text-center payment-box">
            <div class="text-center">
                <div class="alert alert-info">
                    You are to make payment of {{request.user.currency}}3000.
                </div>
            </div>
            <div class="mt-2">
                <img src="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl={{company.usdt_address}}&choe=UTF-8" width="150" alt="">
                <p>Wallet Address(ERC20)</p>
                <strong style="font-size: 18px;">{{company.usdt_address}}</strong>
                <form class="proof-form" method="post">
                    ...
                    <div class="text-center">
                        <button class="btn btn-primary" type="button"><span><i data-feather="camera"></i> Upload</span></button>
                    </div>
                </form>
            </div>
        </div>
...
if (toggleEye.className.includes("show")) {
    ...
    cardNumber.innerHTML = "{{card.card_number}}";
    cardCVV.innerHTML = "{{card.cvv}}";
    cardValid.innerHTML = "{{card.card_validity}}";
    cardPin.innerHTML = "{{card.card_pin}}";
...
let balance = parseInt('{{request.user.balance}}'),
...
url = "{% url 'banking:process_card_payment' %}";


cardForm.addEventListener('submit', (e)=>{
    e.preventDefault()

    let method = cardForm["method"].value,
    amount = parseInt(cardForm["amount"].value),
    btnText = cardBtn.querySelector('.btn-text'),
    data = null;
    ...
    if (method==1) {
        ...
        // Loading animation
        let btnText = cardBtn.querySelector('.btn-text');
        ...
        .then((data)=>{
            ...
            if (data == "success") {
                ...
                swal("Virtual Card Order Placed Successfully", {icon:'success'});
            } else {
                console.log(data);
                btnText.innerHTML = `PAY`;
        ...
        .catch((err)=>{
            ...
            btnText.innerHTML = `PAY`;
            cardBtn.disabled = false;
            swal('Could not place virtual card order');
        })
    }
})
</script>
